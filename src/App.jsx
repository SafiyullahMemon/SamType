...
function App() {
  ...
  const startNewGame = (time = gameTime) => {
    setGameTime(time);
    setTimeLeft(time);
    setStatus('idle');
    generateWords();
  ...
  const handleGameEnd = (wpm, accuracy) => {
    setStatus('finished');
  ...
  return (
    <div className="min-h-screen p-8 transition-colors duration-300 selection:bg-[var(--accent)] selection:text-[var(--bg-color)]">
      <div className="max-w-4xl mx-auto flex flex-col gap-8">
        ...
        {status === 'finished' && (
          <Results wpm={stats.wpm} accuracy={stats.accuracy} />
        )}
      </div>
    </div>
  );
